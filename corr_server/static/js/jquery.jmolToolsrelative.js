...
;(function($) {
    ...
    var jmolModel = {
        ...
        loadData: function() {
            ...
            // This AJAX call gets the relative coordinate data
            $.ajax({
                url: $.fn.jmolTools.options.serverUrlCoordRelative,
                type: 'POST',
                data: {'core' : self.$elem.data($.fn.jmolTools.options.dataAttributeCoordRelative)}
            }).done(function(data) {
        ...
        superimpose: function() {
            ...
            if ( m < 2 ) { return; } // m == 1; nothing to superimpose on
                ...
                var command = 'if ({*.P/' + m + '.3}.length == {*.P/1.3}) ' +
                              '{x=compare({spine/' + m + '.3},{spine/1.3});}' +
                              'else {x=compare({(spine/' + m + '.3)[1][4]},{(spine/1.3)[1][4]});};' +
    ...
    $.fn.jmolTools.options = {
        ...
        serverUrlCoordRelative   : 'http://rna.bgsu.edu/rna3dhub/rest/getCoordinatesRelative',
        dataAttributeCoordRelative: 'core',
